...
OUTPUT_DIR="./flow_outputs/flowC_AuckC"
...
while IFS= read -r line; do
  ...
  ./bin/molasses.ljc ./inputs/molasses_C.conf

  # Check if the flowC0 output file exists
  if [ -f "flowC0" ]; then
    # Define CSV output path
    output_csv="$OUTPUT_DIR/flowC_${X}_${Y}.csv"

    # Run the CSV cleaner script to create a clean CSV
    python3 "$CSV_CLEANER_SCRIPT" "flowC0" "$output_csv"
    ...
    echo "Error: flowC0 file not found for vent at X:$X, Y:$Y"
  fi
